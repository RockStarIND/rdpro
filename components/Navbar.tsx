...
const Navbar = () => {
  ...
  const install = async () => {
    // deferredPrompt is a global variable that gets set 
    // in ~/public/index.js when the beforeinstallprompt event runs
          ...
          {/*siteConfig.links.length !== 0 &&
            siteConfig.links.map((l: { name: string; link: string }) => (
              <a
                key={l.name}
                href={l.link}
                target={/install/i.test(l.name) ? "_self" : "_blank"}
                onClick={/install/i.test(l.name) ? install : () => {}}
                rel="noopener noreferrer"
                className="flex items-center space-x-2 hover:opacity-80 text:black dark:text-white"
              >
                {/install/i.test(l.name) ? '' : <i className="ri-l.name.toLowerCase() as l.name-line ri-lg"></i>}
        ...
        <Dialog as="div" className="fixed inset-0 z-10 overflow-y-auto" open={isOpen} onClose={() => setIsOpen(false)}>
          <div className="min-h-screen px-4 text-center">
            <Transition.Child
              ...
              leaveTo="opacity-0"
            >
              <Dialog.Overlay className="fixed inset-0 bg-white dark:bg-slate-900" />
            </Transition.Child>
            ...
            >
              <div className="my-8 inline-block w-full max-w-md transform overflow-hidden rounded-lg bg-white p-6 text-left align-middle transition-all dark:bg-slate-900">
                ...
                <div className="mt-8 flex items-center justify-end">
                  <button
                    className="mr-3 inline-flex items-center justify-center space-x-2 rounded-lg bg-blue-500 px-4 py-2 text-white hover:bg-blue-400 focus:outline-none focus:ring focus:ring-blue-300"
                    onClick={() => setIsOpen(false)}
                  >
                    {t('Cancel')}
                  </button>
                  <button
                    className="inline-flex items-center justify-center space-x-2 rounded-lg bg-red-500 px-4 py-2 text-white hover:bg-red-400 focus:outline-none focus:ring focus:ring-red-300"
                    onClick={() => clearTokens()}
                  >
