  ...
  return (
    <div className="relative w-24 flex-shrink-0 text-sm text-gray-600 dark:text-gray-300 md:w-28">
      <Listbox value={preferredLayout} onChange={setPreferredLayout}>
        ...
        >
          <Listbox.Options className="absolute right-0 z-20 mt-1 w-32 overflow-auto rounded-lg border border-gray-900/10 bg-white py-1 shadow-lg focus:outline-none dark:border-gray-500/30 dark:bg-slate-900">
            {layouts.map(layout => (
              <Listbox.Option
                key={layout.id}
                className={`${
