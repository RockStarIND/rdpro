...
export const Checkbox: FC<{
  ...
  return (
    <span
      title={title}
      className="inline-flex cursor-pointer items-center rounded-lg p-1.5 hover:bg-gray-300 dark:hover:bg-gray-600"
      onClick={handleClick}
    >
      <input
...
export const Downloading: FC<{ title: string; style: string }> = ({ title, style }) => {
  return (
    <span title={title} className={`${style} rounded-lg`} role="status">
      <LoadingIcon
...
const FileListing: FC<{ query?: ParsedUrlQuery }> = ({ query }) => {
  ...
  const [formattedPath, setFormattedPath] = useState(false);
  ...
  if ('folder' in responses[0]) {
    ...
    folderChildren = folderChildren.filter((child: OdFolderChildren) => !/icon\./i.test(child.name))
      .sort((child: OdFolderChildren, nextChild: OdFolderChildren) => {
        ...
        return child.name.charCodeAt(0) - nextChild.name.charCodeAt(0)
      })

    // Add the path mappings to eliminate the spaces in the url
    lodash.each(folderChildren, (folderChild) => {
      let decodedPath: any; 
      if (lodash.startsWith(path, '/')) {
        decodedPath = path === '/' ? [] : decodeURIComponent(path.substring(1)).split('/');
      }
      else {
        decodedPath = path === '/' ? [] : decodeURIComponent(path).split('/');
      }
      ItemPathStore.setMapping([...decodedPath, folderChild.name], [...decodedPath, folderChild.name.replaceAll(' ', '-')])
    });
    ...
    return (
      <>
        ...
        {layout.name === 'Grid' && entries.hasFoldersOnly ? <FolderGridLayout {...folderProps} /> : <FolderListLayout {...folderProps} />}

        {!onlyOnePage && (
          <div className="rounded-lg-b bg-white dark:bg-slate-900 dark:text-gray-100">
