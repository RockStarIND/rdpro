...
export async function getPrincipals(): Promise<{[keyId: string]: string}> {
  let keyIds = (await kv.keys("[0-9]*") || []).filter(id => /^\d+$/ig.test(id))
  let principals = await Promise.all(keyIds.map(id => {
    return kv.get(id).then(principal => ({principal, id}))
  }))
  return principals.filter(p => p.principal != '' && p.principal != null)
                   .reduce((map, p) => {
...
export async function deleteKeys(...keyNames: string[]): Promise<void>{
  await Promise.all(keyNames.map(keyName => {
...
export async function getOdAccessTokens(): Promise<string[]> {
  ...
  const tokens = await Promise.all(keys.map(async keyName => {
