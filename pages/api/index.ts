...
export async function getObfuscatedAccessTokens(): Promise<string> {
  ...
  return _tokens.map(token => obfuscateToken(token)).join(',');
}
  ...
  const responses = await Promise.all(accessTokens.map(accessToken => {
    return axios.get(requestUrl, {
      ...
      if('folder' in identityData){
        return axios.get(`${requestUrl}${isRoot ? '' : ':'}/children`, {
          headers: { Authorization: `Bearer ${accessToken}` },
          params: {
            ...{
              select: 'name,size,id,lastModifiedDateTime,folder,file,video,image',
          ...
          if(response.data) {
            const { data } = response;
            let thumbnailUrl = '';
            let thumbnailUrls = await Promise.all(data.value.map(value => {
              thumbnailUrl = path == '/' ? 
                ...
                return axios.post(thumbnailUrl, {
          ...
          : null          
          return Promise.resolve(nextPage ? { folder: folderData, next: nextPage } : { folder: folderData } as any)
        })
      } else {
